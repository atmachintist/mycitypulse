...
export default function ElectionsCard({ election, cityName }) {
  ...
  // Calculate countdown timer (DD:HH:MM:SS)
  useEffect(() => {
  ...
  // Days until election (for display)
  ...
  // Handle pledge share on social media
  const sharePledgeOnSocial = (platform) => {
    ...
    const urls = {
      ...
      instagram: `https://www.instagram.com/` // Instagram doesn't support direct text share
    };
      ...
      <div className="elections-snapshot">
        ...
        <div className="elections-status">
          ...
          <p className="timer-display">
            ⏱️ <strong>{countdown}</strong> minutes to elections
          </p>
          ...
          {!hasVoted ? (
            <>
              <button
                ...
                onClick={handlePledgeVote}
                aria-label="Pledge to vote"
              >
                ✋ I Will Vote
              </button>
        ...
        {showPledgeShare && (
          <div className="pledge-share-modal">
            ...
            <button
              className="close-pledge-modal"
              onClick={() => setShowPledgeShare(false)}
            >
              Done
            </button>
          </div>
